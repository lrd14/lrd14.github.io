  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (action === "redeem") {
        ...
        const turnstileToken = String(body.turnstileToken || "").trim();
        if (!token) {
          return json({ success: false, message: "Missing session token." }, 400, env);
        }
        if (!license) {
          return json({ success: false, message: "License key is required." }, 400, env);
        }
        if (!turnstileToken) {
          return json({ success: false, message: "Verification is required." }, 400, env);
        }
        ...
        return json(
          {
            success: true,
            username: payload.u,
            message: result.message || "License key redeemed successfully."
          },
          200,
          env
        );
